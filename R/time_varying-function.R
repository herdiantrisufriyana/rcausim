...
#' directed cyclic graph (DCG), which means loops are allowed. Use
#' \code{edge_from_function} to identify edges given a list of functions, then
#' draw a causal diagram using the edges data frame (see vignettes). All
#' arguments within any function must be defined by their respective functions,
#' except the argument 'n'. The output lengths of vertex functions must match
#' the 'data' row number.
#' @param data Data, a data frame generated by \code{data_from_function} which
#' contains causally-simulated data at \(t_0\). Column names of 'i', 't', and
