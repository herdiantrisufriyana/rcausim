...
#' functions. The causal structure needs to be a directed acyclic graph (DAG),
#' which means no loops are allowed. Use \code{\link{edge_from_function}} to
#' identify edges given a list of functions, then draw a causal diagram using
#' the edges data frame (see vignettes). At least a function in the list must
#' include 'n' as the only argument. All arguments within any function must be
#' defined by their respective functions, except the argument 'n'. The output
#' lengths of vertex functions must match the specified length 'n'.
